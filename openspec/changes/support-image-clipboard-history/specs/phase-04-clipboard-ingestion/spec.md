...
The system SHALL listen for clipboard changes in background mode, normalize supported content types (including text and image payloads), and safely enqueue entries for persistence.
...
#### Scenario: Image payloads are normalized for persistence
- **WHEN** an image payload is observed on the clipboard
- **THEN** ingestion MUST normalize it into an image entry candidate with a persisted temporary-file reference
